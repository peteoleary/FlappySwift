...
class GameScene: SKScene, SKPhysicsContactDelegate{
    ...
    override func didMove(to view: SKView) {
        ...
        for i:Int in 0 ..< Int(2.0 + self.frame.size.width / ( groundTexture.size().width * 2.0 ) + 1) {
        ...
        for i:Int in 0 ..< Int(2.0 + self.frame.size.width / ( skyTexture.size().width * 2.0 ) + 1) {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        if moving.speed > 0 {
            if ( contact.bodyA.categoryBitMask & scoreCategory ) == scoreCategory || ( contact.bodyB.categoryBitMask & scoreCategory ) == scoreCategory {
                ...
                let myDict : Dictionary<String, String> = ["Pipe Passed": String(score)]
                KISSmetricsAPI.shared().record("/pipe_passed", withProperties: myDict)
